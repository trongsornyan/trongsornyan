# **Hi there! 👋 I'm Tammy**  

### **Data Analyst | Quantitative Economist | Project Manager**  

I’m passionate about transforming complex data into actionable insights that drive business impact. With a background in Business Economics and a Master of Data Analytics from QUT, I bridge the gap between business needs and data solutions. My journey from theoretical frameworks to real-world applications has equipped me with a unique perspective on problem-solving.  

---

### 🔍 **What I Do**  
- **Data Analysis & Modeling:** Using Python, SQL, and Excel to extract meaningful insights.  
- **Data Visualization & Storytelling:** Crafting compelling narratives with Tableau.  
- **Strategic Decision Making:** Applying econometrics and statistical analysis for data-driven solutions.  
- **Project Management:** Driving projects with strategic alignment and stakeholder collaboration.
- 
---

### 💼 **Experience & Background**  
- **Business Economics & Data Analytics:** Bridging numbers with narratives.  
- **FMCG Food Industry:** Managed projects ensuring operational efficiency and strategic alignment.  
- **Project Management:** Coordinated cross-functional teams to deliver impactful solutions.  

---

### 🛠️ **Tech Stack**  
- **Programming & Data Analysis:** Python, SQL, Excel  
- **Data Visualization:** Tableau, Matplotlib, Seaborn  
- **Other Tools:** Git, Jupyter Notebooks  

---

### 📫 **Let's Connect!**  
- **[LinkedIn](https://www.linkedin.com/in/trongsornyan/)**  
- **[Portfolio Website](https://tammy-portfolio.notion.site/TAMMY-S-DATA-PORTFOLIO-19833cc1e32c80f6a884c93f6b0cd311?pvs=4)**  
- **[Email](mailto:trongsorn.yan@gmail.com)**  
